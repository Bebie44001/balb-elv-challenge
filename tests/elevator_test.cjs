...
describe('Elevator', function() {
  ...
  it('Level 7: dispatchEfficient should traverse fewer floors than dispatch (4 scenarios)', () => {
    // Make results deterministic regardless of current time (ignore return-to-lobby behavior).
    elevator.checkReturnToLoby = () => false

    const scenarios = [
      // A requested first, but B is much closer to current elevator position (0).
      // Baseline FCFS tends to go to A first, then come back for B.
      { A: new Person('A1', 8, 9), B: new Person('B1', 1, 0) },
      { A: new Person('A2', 9, 8), B: new Person('B2', 2, 0) },

      // A is far above, B is far below (and close to 0). Efficient should serve B first.
      { A: new Person('A3', 10, 11), B: new Person('B3', 1, 0) },
      { A: new Person('A4', 7, 9), B: new Person('B4', 1, 0) },
    ]

    scenarios.forEach(({ A, B }) => {
      elevator.reset()
      elevator.checkReturnToLoby = () => false
      elevator.requests = [A, B]
      elevator.dispatch()
      ...
      const efficientFloors = elevator.floorsTraversed

      assert.isBelow(efficientFloors, baselineFloors)
    })
  })
});

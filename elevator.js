export default class Elevator {
  ...
  dispatchEfficient(){
    while(this.requests.length || this.riders.length){
      ...
      const candidateFloors = [...pickupFloors, ...dropOffFloors]

      if(!candidateFloors.length) break

      const nextFloor = candidateFloors.reduce((closest, floor) => {
        const closestDist = Math.abs(closest - this.currentFloor)
        const floorDist = Math.abs(floor - this.currentFloor)
        if(floorDist < closestDist) return floor
        if(floorDist > closestDist) return closest
        return floor < closest ? floor : closest
      }, candidateFloors[0])

      while(this.currentFloor !== nextFloor){
        if(nextFloor > this.currentFloor) this.moveUp()
        else this.moveDown()
      }
    }
